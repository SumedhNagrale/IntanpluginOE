...
Rhd2000EvalBoard::Rhd2000EvalBoard()
{
    ...
    std::cout << "Rhd2000EvalBoard::Rhd2000EvalBoard" << std::endl;
...
int Rhd2000EvalBoard::open(const char* libname)
{
    ...
    std::cout << "Rhd2000EvalBoard::open" << std::endl;
    ...
    if (usb3)
        type = ControllerType::ControllerOEOpalKellyUSB3;
    else
        type = ControllerType::ControllerOEOpalKellyUSB2;
...
void Rhd2000EvalBoard::initialize()
{
    ...
    for (int i = 1; i < MAX_NUM_DATA_STREAMS_USB3; i++) {
...
double Rhd2000EvalBoard::getSampleRate() const
{
    ...
    //std::cout << "Rhd2000EvalBoard::getSampleRate()" << std::endl;
...
void Rhd2000EvalBoard::selectAuxCommandBank(BoardPort port, AuxCmdSlot auxCommandSlot, int bank)
{
   ...
   // std::cout << "Rhd2000EvalBoard::selectAuxCommandBank " << port << " " << auxCommandSlot << " " << bank << std::endl;
...
void Rhd2000EvalBoard::selectAuxCommandLength(AuxCmdSlot auxCommandSlot, int loopIndex, int endIndex)
{
    ...
    //std::cout << "Rhd2000EvalBoard::selectAuxCommandLength " << auxCommandSlot << " " << loopIndex << " " << endIndex << std::endl;
...
void Rhd2000EvalBoard::resetBoard()
{
    ...
    std::cout << " Rhd2000EvalBoard::resetBoard()" << std::endl;
...
{
    dev->UpdateWireOuts();
    unsigned int a = dev->GetWireOutValue(WireOutNumWordsMsb);
    unsigned int b = dev->GetWireOutValue(WireOutNumWordsLsb);
    return ( (a << 16) + b);
}
...
void Rhd2000EvalBoard::setDataSource(int stream, BoardDataSource dataSource)
{
    ...
    std::cout << "Rhd2000EvalBoard::setDataSource(" << stream << ", " << dataSource << ")" << std::endl;


    if (stream < 0 || stream > (MAX_NUM_DATA_STREAMS_USB3 - 1)) {
...
void Rhd2000EvalBoard::enableDataStream(int stream, bool enabled)
{
    ...
    std::cout << "Rhd2000EvalBoard::enableDataStream(" << stream << ", " << enabled << ")" << std::endl;
...
// was available.
bool Rhd2000EvalBoard::readDataBlock(RHXDataBlock *dataBlock)
{
...
int Rhd2000EvalBoard::getCableDelay(BoardPort port) const
{
    ...
    std::cout << " Rhd2000EvalBoard::getCableDelay(BoardPort port)" << std::endl;
...
void Rhd2000EvalBoard::getCableDelay(std::vector<int> &delays) const
{
    ...
    std::cout << " Rhd2000EvalBoard::getCableDelay(std::vector<int> &delays)" << std::endl;
...
void Rhd2000EvalBoard::resetFpga()
{
    ...
    std::cout << " Rhd2000EvalBoard::resetFpga()" << std::endl;
...
void Rhd2000EvalBoard::enableBoardLeds(bool enable)
{
    ...
    std::cout << " Rhd2000EvalBoard::enableBoardLeds()" << std::endl;
...
void Rhd2000EvalBoard::setClockDivider(int divide_factor)
{
    ...
    std::cout << " Rhd2000EvalBoard::setClockDivider()" << std::endl;
...
{
    dev->UpdateWireOuts();
    int a = dev->GetWireOutValue(0x28);
    int b = dev->GetWireOutValue(0x2a);
    int c = dev->GetWireOutValue(0x29);
    std::cout << "In FIFO: " << a << " DDR: " << b << " Out FIFO: " << c << std::endl;
}
